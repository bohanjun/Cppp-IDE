...
open class Highlightr {
    ...
    /**
     ...
     - parameter highlightPath: The path to `highlight.min.js`.
    ...
    public init?(highlightPath: String? = nil) {
        ...
        var hgJs: String = "Error"
        do {
            hgJs = try String(contentsOfFile: hgPath)
        } catch {
            print("error")
        }
    ...
    open func setTheme(to name: String) -> Bool {
        guard let defTheme = bundle.path(forResource: name.lowercased() + ".min", ofType: "css") else {
            return false
        }
